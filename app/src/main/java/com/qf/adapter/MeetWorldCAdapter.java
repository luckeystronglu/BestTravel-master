 ...
 */
public class MeetWorldCAdapter extends RecyclerView.Adapter {
...
//    private Intent intent;
    ...
    public MeetWorldCAdapter(Context context) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == 0) {
            ...
            return new NoteHolder1(view);
        }

        View view = LayoutInflater.from(context).inflate(R.layout.item_activity_morenote, parent, false);
        return new NoteHolder2(view);
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        //设置数据
        if (holder instanceof NoteHolder1) {
            if (datas.get(position).getTitle() != null) {
                ((NoteHolder1) holder).meet_con_title.setText(datas.get(position).getTitle());
            }else {
                ((NoteHolder1) holder).meet_con_title.setVisibility(View.GONE);
            }

            ((NoteHolder1) holder).meet_con_summary.setText(datas.get(position).getDescription());
        }
        else if (holder instanceof NoteHolder2) {
            ((NoteHolder2) holder).username.setText(datas.get(position).getUser_activity().getUser().getName());


            ((NoteHolder2) holder).tv_note_title.setText(datas.get(position).getUser_activity().getTopic());

            ((NoteHolder2) holder).expandableTextView.setText(datas.get(position).getUser_activity().getDescription(), mCollapsedStatus, position);
...
//        });
            ...
            ((NoteHolder2) holder).tv_likesnum.setText(datas.get(position).getUser_activity().getLikes_count() + "");
            ((NoteHolder2) holder).tv_commentnum.setText(datas.get(position).getUser_activity().getComments_count() + "");
            ((NoteHolder2) holder).tv_favoritenum.setText(datas.get(position).getUser_activity().getFavorites_count() + "");

            //添加标签
            ((NoteHolder2) holder).ll_marks.removeAllViews();//移除线性布局中的所有控件
            ...
            if (markslist.size() != 0) {
                for (int i = 0; i < markslist.size(); i++) {
                    ...
                    ((NoteHolder2) holder).ll_marks.addView(tv, llmarkparams);
                }
            }

            if (datas.get(position).getUser_activity().getPoi() != null && !datas.get(position).getUser_activity().getPoi().equals("null")) {
                if (datas.get(position).getUser_activity().getPoi().getName() != null) {
                    ...
                    tv.setPadding(12, 4, 12, 4);
                    tv.setBackgroundResource(R.drawable.tv_note_marks_shape);
                    ((NoteHolder2) holder).ll_marks.addView(tv, llmarkparams2);
                }

            }

            List<MeetWorldEntity.DataBean.ItemsBean.UserActivityBean.CategoriesBean> catelist = datas.get(position).getUser_activity().getCategories();
            if (catelist.size() != 0) {
                for (int i = 0; i < catelist.size(); i++) {
                    ...
                    tv.setBackgroundResource(R.drawable.tv_note_marks_shape);
//                tv.getLayoutParams();
                    ((NoteHolder2) holder).ll_marks.addView(tv, llmarkparams3);
                }
            }
            ...
            if (datas.get(position).getUser_activity().getMade_at() != null) {
                ...
                tv.setBackgroundResource(R.drawable.tv_note_marks_shape);
                ((NoteHolder2) holder).ll_marks.addView(tv, llmarkparams4);
            }


            List<MeetWorldEntity.DataBean.ItemsBean.UserActivityBean.ContentsBean> imglist = datas.get(position).getUser_activity().getContents();
//        intent = new Intent(context,MNoteLookPicsActivity.class);
            ...
            Glide.with(context)
                    ...
                    .thumbnail(0.1f)
                    .into(((NoteHolder2) holder).bigimg);
      ...
      /*  holder.bigimg.setOnClickListener(new View.OnClickListener() {
            ...
            //添加scrollview图片
            ((NoteHolder2) holder).ll_img.removeAllViews();
            ...
            if (imglist.size() != 0) {
                for (int i = 1; i < imglist.size(); i++) {
                    ...
                    Glide.with(context)
                            .load(datas.get(position).getUser_activity().getContents().get(i).getPhoto_url())
                            .crossFade(500)
                            .diskCacheStrategy(DiskCacheStrategy.ALL)
                    ...
                    ((NoteHolder2) holder).ll_img.addView(img, llimgparams);
...
//        Glide下载用户头像
            Glide.with(context)
                    ...
                    .thumbnail(0.1f)
                    .into(((NoteHolder2) holder).userimg);
    ...
    public class NoteHolder1 extends RecyclerView.ViewHolder {
        TextView meet_con_title, meet_con_summary;

        public NoteHolder1(View itemView) {
            super(itemView);
            this.meet_con_title = (TextView) itemView.findViewById(R.id.meet_con_title);
            this.meet_con_summary = (TextView) itemView.findViewById(R.id.meet_con_summary);
        }
    }

    public class NoteHolder2 extends RecyclerView.ViewHolder {
        ...
        public NoteHolder2(View itemView) {
            super(itemView);
